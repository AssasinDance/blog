import './content.scss'
// import { useEffect, useState } from 'react'
import { Pagination } from 'antd'

// import { getArticles } from '../../services/blog-platform-api'
import { useGetArticlesQuery } from '../../services/api'

import Article from './article/article'
// import { Pagination } from './pagination/pagination'

export default function Content({ user, currentPage, setCurrentPage }) {
  // const [articles, setArticles] = useState(null)
  const { data, error, isLoading } = useGetArticlesQuery({
    page: currentPage,
    token: user !== null ? user.token : user,
  })

  // useEffect(() => {
  //   getArticles().then((data) => {
  //     setArticles(data)
  //   })
  // }, [setArticles])

  // function paginationHandler(page) {
  //   setArticles(null)
  //   getArticles(page)
  //     .then((data) => {
  //       setArticles(data)
  //       setCurrentPage(page)
  //     })
  //     .catch(() => {
  //       setArticles('error')
  //     })
  // }

  if (error) {
    return (
      <main className="content">
        <div className="content__error">
          Произошла непредвиденная ошибка!
          <br />
          Попробуйте перезагрузить страницу и проверить интернет-соединение
        </div>
      </main>
    )
  }

  if (isLoading) {
    return (
      <main className="content">
        <div className="loading-spinner">
          <div className="loading">
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
            <div />
          </div>
        </div>
      </main>
    )
  }

  if (data) {
    return (
      <main className="content">
        {data.articles.map((article) => {
          return <Article key={article.slug} article={article} user={user} />
        })}
        <Pagination
          className="content__pagination"
          align="center"
          current={currentPage}
          defaultCurrent={1}
          total={data.articlesCount}
          showSizeChanger={false}
          onChange={(page) => setCurrentPage(page)}
        />
      </main>
    )
  }

  return null
}
